...
use tiff::encoder::value::{Ifd, Ifd8, SRational};
...
fn encode_decode() {
    ...
    let mut file = Cursor::new(Vec::new());
    {
        ...
        tiff.set_tag(Tag::Artist, "Image-tiff");

        let image = tiff
            .new_image(100, 100, PhotometricInterpretation::RGB)
            .unwrap();
        image.write(&image_data).unwrap();
    }
    std::fs::write("test.tiff", file.get_ref()).unwrap();
    {
...
fn encode_decode_big() {
    ...
    let mut file = Cursor::new(Vec::new());
    {
        let mut tiff = TiffEncoder::new_big(&mut file);
...
fn test_encode_ifd() {
    ...
    {
        let mut tiff = TiffEncoder::new(&mut data);

        // Use the "reusable" tags section as per the TIFF6 spec
        tiff.set_tag(Tag::Unknown(65000), Ifd(42u32));
        tiff.set_tag(Tag::Unknown(65001), &[Ifd(100u32)][..]);
        tiff.set_tag(Tag::Unknown(65002), &[Ifd(1u32), Ifd(2u32), Ifd(3u32)][..]);

        tiff.set_tag(Tag::Unknown(65010), Ifd8(43u64));
        tiff.set_tag(Tag::Unknown(65011), &[Ifd8(100u64)][..]);
        tiff.set_tag(
            Tag::Unknown(65012),
            &[Ifd8(1u64), Ifd8(2u64), Ifd8(3u64)][..],
        );

        let image_encoder = tiff
            .new_image(1, 1, PhotometricInterpretation::BlackIsZero)
            .unwrap();
        image_encoder.write_encoded(&[&[1]]).unwrap();
    }
...
fn test_encode_undersized_buffer() {
    ...
    let mut tiff = TiffEncoder::new(&mut output_stream);
    let res = tiff.write_image(50, 50, PhotometricInterpretation::RGB, &input_data);
    assert!(res.is_err());
}
...
macro_rules! test_roundtrip {
    ($name:ident, $buffer:ident, $buffer_ty:ty, $format:ident) => {
        fn $name(file: &str, expected_type: ColorType) {
            ...
            let mut file = Cursor::new(Vec::new());
            {
                ...
                tiff.set_bits_per_sample(mem::size_of::<$buffer_ty>() as u8 * 8);
                tiff.set_sample_format(SampleFormat::$format);
                tiff.write_image(
                    width,
                    height,
...
test_roundtrip!(test_u8_roundtrip, U8, u8, Uint);
test_roundtrip!(test_i8_roundtrip, I8, i8, Int);
test_roundtrip!(test_u16_roundtrip, U16, u16, Uint);
test_roundtrip!(test_i16_roundtrip, I16, i16, Int);
test_roundtrip!(test_u32_roundtrip, U32, u32, Uint);
test_roundtrip!(test_u64_roundtrip, U64, u64, Uint);
test_roundtrip!(test_f32_roundtrip, F32, f32, IEEEFP);
test_roundtrip!(test_f64_roundtrip, F64, f64, IEEEFP);

#[test]
fn test_gray_u8_roundtrip() {
    test_u8_roundtrip("minisblack-1c-8b.tiff", ColorType::Gray(8));
}

#[test]
fn test_gray_i8_roundtrip() {
    test_i8_roundtrip("minisblack-1c-i8b.tiff", ColorType::Gray(8));
}

#[test]
fn test_rgb_u8_roundtrip() {
    test_u8_roundtrip("rgb-3c-8b.tiff", ColorType::RGB(8));
}

#[test]
fn test_cmyk_u8_roundtrip() {
    test_u8_roundtrip("cmyk-3c-8b.tiff", ColorType::CMYK(8));
}

#[test]
fn test_gray_u16_roundtrip() {
    test_u16_roundtrip("minisblack-1c-16b.tiff", ColorType::Gray(16));
}

#[test]
fn test_gray_i16_roundtrip() {
    test_i16_roundtrip("minisblack-1c-i16b.tiff", ColorType::Gray(16));
}

#[test]
fn test_rgb_u16_roundtrip() {
    test_u16_roundtrip("rgb-3c-16b.tiff", ColorType::RGB(16));
}

#[test]
fn test_cmyk_u16_roundtrip() {
    test_u16_roundtrip("cmyk-3c-16b.tiff", ColorType::CMYK(16));
}

#[test]
fn test_gray_u32_roundtrip() {
    test_u32_roundtrip("gradient-1c-32b.tiff", ColorType::Gray(32));
}

#[test]
fn test_rgb_u32_roundtrip() {
    test_u32_roundtrip("gradient-3c-32b.tiff", ColorType::RGB(32));
}

#[test]
fn test_gray_u64_roundtrip() {
    test_u64_roundtrip("gradient-1c-64b.tiff", ColorType::Gray(64));
}

#[test]
fn test_rgb_u64_roundtrip() {
    test_u64_roundtrip("gradient-3c-64b.tiff", ColorType::RGB(64));
}

#[test]
fn test_gray_f32_roundtrip() {
    test_f32_roundtrip("gradient-1c-32b-float.tiff", ColorType::Gray(32));
}

#[test]
fn test_rgb_f32_roundtrip() {
    test_f32_roundtrip("gradient-3c-32b-float.tiff", ColorType::RGB(32));
}

#[test]
fn test_cmyk_f32_roundtrip() {
    test_f32_roundtrip("cmyk-3c-32b-float.tiff", ColorType::CMYK(32));
}

#[test]
fn test_gray_f64_roundtrip() {
    test_f64_roundtrip("gradient-1c-64b-float.tiff", ColorType::Gray(64));
}

#[test]
fn test_ycbcr_u8_roundtrip() {
    test_u8_roundtrip("tiled-jpeg-ycbcr.tif", ColorType::YCbCr(8));
}
...
fn test_multiple_byte() {
    ...
    {
        let mut tiff = TiffEncoder::new(&mut data);
        tiff.set_tag(Tag::Unknown(65000), &[1_u8][..]);
        tiff.set_tag(Tag::Unknown(65001), &[1_u8, 2][..]);
        tiff.set_tag(Tag::Unknown(65002), &[1_u8, 2, 3][..]);
        tiff.set_tag(Tag::Unknown(65003), &[1_u8, 2, 3, 4][..]);
        tiff.set_tag(Tag::Unknown(65004), &[1_u8, 2, 3, 4, 5][..]);
        tiff.new_image(1, 1, PhotometricInterpretation::BlackIsZero)
            .unwrap()
            .write_encoded(&[&[1]])
            .unwrap();
    }
...
fn test_signed() {
...

    {
        ...
        //Use the "reusable" tags section as per the TIFF6 spec
        tiff.set_tag(Tag::Unknown(65000), -1_i8);
        tiff.set_tag(Tag::Unknown(65001), &[-1_i8][..]);
        tiff.set_tag(Tag::Unknown(65002), &[-1_i8, 2][..]);
        tiff.set_tag(Tag::Unknown(65003), &[-1_i8, 2, -3][..]);
        tiff.set_tag(Tag::Unknown(65004), &[-1_i8, 2, -3, 4][..]);
        tiff.set_tag(Tag::Unknown(65005), &[-1_i8, 2, -3, 4, -5][..]);

        tiff.set_tag(Tag::Unknown(65010), -1_i16);
        tiff.set_tag(Tag::Unknown(65011), -1_i16);
        tiff.set_tag(Tag::Unknown(65012), &[-1_i16, 2][..]);
        tiff.set_tag(Tag::Unknown(65013), &[-1_i16, 2, -3][..]);

        tiff.set_tag(Tag::Unknown(65020), -1_i32);
        tiff.set_tag(Tag::Unknown(65021), &[-1_i32][..]);
        tiff.set_tag(Tag::Unknown(65022), &[-1_i32, 2][..]);

        tiff.set_tag(Tag::Unknown(65030), -1_i64);
        tiff.set_tag(Tag::Unknown(65031), &[-1_i64][..]);
        tiff.set_tag(Tag::Unknown(65032), &[-1_i64, 2][..]);

        tiff.set_tag(Tag::Unknown(65040), make_srational(-1));
        tiff.set_tag(
            Tag::Unknown(65041),
            &[make_srational(-1), make_srational(2)][..],
        );

        tiff.write_image(1, 1, PhotometricInterpretation::BlackIsZero, &[1])
            .unwrap();
    }
...
fn test_multipage_image() {
...

    {
        ...
        let mut img_encoder = TiffEncoder::new(&mut img_file);

        // write first grayscale image (2x2 16-bit)
        let img1: Vec<u16> = [1, 2, 3, 4].to_vec();
        img_encoder.set_bits_per_sample(16);
        img_encoder
            .write_image(
                2,
                2,
                PhotometricInterpretation::BlackIsZero,
                bytemuck::cast_slice(&img1),
            )
            .unwrap();
        ...
        img_encoder.set_bits_per_sample(16);
        img_encoder
            .write_image(3, 3, PhotometricInterpretation::BlackIsZero, &img2[..])
            .unwrap();
    }
...
fn test_rows_per_strip() {
    let mut file = Cursor::new(Vec::new());
    {
        ...
        img_encoder.set_rows_per_strip(2);
        let image = img_encoder
            .new_image(100, 100, PhotometricInterpretation::BlackIsZero)
            .unwrap();

        let img2 = vec![0; 2 * 100];
        let strips = vec![&*img2; 50];
        image.write_encoded(&*strips).unwrap();
    }
